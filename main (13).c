...
struct Platform* Platform_Create(char** Map, int x, int y, int Rows, int Colls){
  ...
  temp = Head_block->next;
    ...
    platform->link_array[0] = malloc(1 * sizeof(struct Links)); // Allocate memory for each link_array element
    platform->link_array[0]->neighbor = NULL; // Initialize neighbor pointer
    platform->link_array[0]->distance = 0; // Initialize distance
    platform->link_array[0]->index = 0; // Initialize index
